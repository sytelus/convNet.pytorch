...
def main():
    ...
    if 'cuda' in args.device:
        if torch.cuda.is_available():
            ...
            multi_device = len(args.device_ids) > 1
        else:
            logging.warning("CUDA was requested but not available.")
...
# forward pass used by train() and validate()
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    for i, (inputs, label) in enumerate(data_loader):
        ...
        if type(output) is list: # might be the case if we have single scaler
            output = output[0]
